  ...
  const msg = {
    ...
    de_DE: {
      ...
      options: {
        ...
        ram: 'Bei gelben Berichten (Verluste) Vorlage B (Rammböcke) aus dem nächstgelegenen Dorf senden:',
      ...
      if (mobileCheck) {
        ...
        table.each((i, el) => {
          try {
            ...
            const ramIndex = game_data.units.indexOf('ram');
            const ram = ramIndex !== -1 ? units[ramIndex] || 0 : 0;
        ...
        $html
          ...
          .map((i, el) => {
            ...
            let ramIndex = game_data.units.indexOf('ram');
            let ram = ramIndex !== -1 ? allUnits[ramIndex] || 0 : 0;
      ...
      if ($.isEmptyObject(data.farms.templates)) {
        ...
        $html
          ...
          .map((i, el) => {
            ...
            // Rammböcke gezielt über das name-Attribut auslesen statt über
            // die Feld-Position, da nicht jede Vorlage die gleichen
            // Einheitentypen anzeigt (z.B. nur wenn Rammböcke konfiguriert sind).
            let ramInput = $inputs.filter((index, element) => {
              return (
                $(element).attr('name').trim().split('[')[0] == 'ram'
              );
            });
            let ram =
              ramInput.length > 0 ? ramInput.first().val().toNumber() : 0;
  ...
  const createPlanning = function (
    ...
    if (optionRam && data.farms.templates.hasOwnProperty('b')) {
      ...
      yellowTargets.forEach((targetCoord) => {
        ...
        for (let i = 0; i < orderedOrigins.length; i++) {
          ...
          // Rammböcke werden separat geprüft, da sie in der normalen
          // units-Liste der Dörfer nicht enthalten sind (siehe skipUnits).
          let ramAvailable =
            (data.villages[originCoord].ram || 0) >= (templateB.ram || 0);
          if (!unitsLeft || !ramAvailable) continue;
          ...
          if (timeDiff) {
            ...
            // Verplante Rammböcke abziehen, damit dasselbe Dorf sie nicht
            // ein zweites Mal für ein anderes gelbes Ziel verwendet.
            data.villages[originCoord].ram =
              (data.villages[originCoord].ram || 0) - (templateB.ram || 0);
            break;
          }
        }
    ...
    for (let prop in data.villages) {
      ...
      orderedFarms.forEach((el) => {
        ...
        // Auch hier: Rammböcke separat prüfen, falls Vorlage B (z.B. über
        // die Maxloot-Option) welche enthält.
        let ramAvailable =
          (data.villages[prop].ram || 0) >= (template.ram || 0);
        ...
        if (unitsLeft && ramAvailable && timeDiff && distance < optionDistance) {
          plan.counter++;
