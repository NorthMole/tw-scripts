  ...
  const msg = {
    nl_NL: {
      ...
      options: {
        ...
        ram: 'Verstuur bij gele rapporten sjabloon B met rammen of katapulten om de muur te vernietigen',
        button: 'Plan farms',
      },
      table: {
        noFarmsPlanned:
    ...
    hu_HU: {
      ...
      options: {
        ...
        ram: 'SÃ¡rga jelentÃ©s esetÃ©n kÃ¼ldje a B sablont faltÃ¶rÅ‘ kossal vagy katapulttal a fal lerombolÃ¡sÃ¡hoz',
    ...
    de_DE: {
      ...
      options: {
        title: 'FarmGod Einstellungen',
        warning:
          '<b>Warnung:</b><br>- Stelle sicher, dass A als deine Standard-Mikrofarm und B als größere Mikrofarm eingestellt ist<br>- Stelle sicher, dass die Farm-Filter korrekt eingestellt sind, bevor du das Skript benutzt',
        filterImage:
          'https://higamy.github.io/TW/Scripts/Assets/farmGodFilters.png',
        ...
        losses: 'Farmen zu Dörfern mit teilweisen Verlusten senden:',
        maxloot: 'Vorlage B senden, wenn die letzte Beute voll war:',
        newbarbs: 'Neue Barbarendörfer zum Farmen hinzufügen:',
        ram: 'Bei gelben Berichten Vorlage B mit Rammböcken oder Katas senden, um den Wall zu zerstören',
        button: 'Farmen planen',
      },
      table: {
        noFarmsPlanned:
          'Mit den gewählten Einstellungen können keine Farmen verschickt werden.',
        origin: 'Herkunft',
        target: 'Ziel',
        fields: 'Felder',
        farm: 'Farm',
    ...
    int: {
      ...
      options: {
        ...
        ram: 'Send ram (template B) from nearest village on partial losses:',
        button: 'Plan farms',
      },
      table: {
        noFarmsPlanned:
...
window.FarmGod.Main = (function (Library, Translation) {
        ...
        return `<style>#popup_box_FarmGod{text-align:center;width:550px;}</style>
            ...
            : ``
                ...
                <div style="width:90%;margin:auto;background: url(\'graphic/index/main_bg.jpg\') 100% 0% #E3D5B3;border: 1px solid #7D510F;border-collapse: separate !important;border-spacing: 0px !important;"><table class="vis" style="width:100%;text-align:left;font-size:11px;">
                  ...
                  <tr><td>${t.options.ram
          }</td><td><input type="checkbox" class="optionRam" ${options.optionRam ? 'checked' : ''
          }></td></tr>
                  ${game_data.market == 'nl'
      ...
      if (mobileCheck) {
        ...
        $html
          ...
          .map((i, el) => {
            ...
            // allUnits: ungefiltert, Index entspricht game_data.units
            // (enthält also auch Rammböcke, die units[] weiter unten nicht hat)
            let allUnits = $el
              .find('.unit-item')
              .map((index, element) => {
      ...
      if ($.isEmptyObject(data.farms.templates)) {
        ...
        $html
          ...
          .map((i, el) => {
            ...
            // Belagerungswaffen (Rammböcke + Katapulte) gezielt über das
            // name-Attribut auslesen statt über die Feld-Position, da nicht
            // jede Vorlage die gleichen Einheitentypen anzeigt.
            let siege = {};
            ['ram', 'catapult'].forEach((unit) => {
  ...
  const createPlanning = function (
    ...
    // Prüft, ob ein Dorf genug Belagerungswaffen (Rammböcke UND Katapulte)
    // für die Vorlage hat. Diese Einheiten stehen nicht in der normalen
    // units-Liste (siehe skipUnits) und müssen separat verglichen werden.
    ...
    // Zieht die verplanten Belagerungswaffen vom Dorf ab, damit sie nicht
    // ein zweites Mal für ein anderes Ziel verplant werden.
    ...
    // Legt einen geplanten Farm-Angriff im plan-Objekt ab und aktualisiert
    // Einheiten des Ursprungsdorfs sowie die bekannten Angriffszeiten des Ziels.
    const addFarmToPlan = (
      originCoord,
      targetCoord,
      template_name,
      ...
      plan.farms[originCoord].push({
        ...
        target: { coord: targetCoord, id: data.farms.farms[targetCoord].id },
        fields: distance,
        template: { name: template_name, id: template.id },
      });

      data.villages[originCoord].units = lib.subtractArrays(
    ...
    // ---- Vorlauf: gelbe Berichte (Verluste) zuerst mit Vorlage B (Rammböcke) ----
    // Ziel: Für jedes Dorf mit einem gelben Bericht (vorherige Teilverluste) wird
    // gezielt und mit höchster Priorität das NÄCHSTGELEGENE eigene Dorf gesucht,
    // das genug Einheiten für Vorlage B hat. So kommt der Rammbock-Trupp mit der
    // kürzesten Laufzeit an und zerstört die Mauer, bevor der Rest der normalen
    // Planung läuft. Vorlage B muss der Nutzer im Spiel selbst mit Rammböcken
    // bestücken - das Skript entscheidet nur, WOHER und WANN sie geschickt werden.
    if (optionRam && data.farms.templates.hasOwnProperty('b')) {
      let templateB = data.farms.templates['b'];

      let yellowTargets = Object.keys(data.farms.farms).filter((coord) => {
        return data.farms.farms[coord].color == 'yellow';
      });

      yellowTargets.forEach((targetCoord) => {
        ...
        for (let i = 0; i < orderedOrigins.length; i++) {
          ...
          // Belagerungswaffen (Rammböcke/Katapulte) separat prüfen, da sie
          // in der normalen units-Liste der Dörfer nicht enthalten sind.
          ...
          if (timeDiff) {
            addFarmToPlan(originCoord, targetCoord, 'b', distance, arrival);
            // Verplante Belagerungswaffen abziehen, damit dasselbe Dorf sie
            // nicht ein zweites Mal für ein anderes gelbes Ziel verwendet.
            subtractSiege(data.villages[originCoord], templateB);
            break;
          }
        }

        // Wichtig: Das gelbe Ziel bleibt bewusst in data.farms.farms!
        // Der B-Angriff (Rammböcke) läuft nur zusätzlich, um den Wall zu
        // zerstören - die normale Planung darf das Dorf davon unabhängig
        // weiterhin (auch mehrfach) mit Vorlage A anfarmen. Pro Durchlauf
        // wird aber maximal EIN B-Angriff pro gelbem Dorf geplant (break).
      });
    }

    // ---- Normale Planung (unverändert) für alle übrigen Ziele ----
      ...
      orderedFarms.forEach((el) => {
        ...
        let template_name =
          optionMaxloot &&
            ...
            ? 'b'
            : 'a';
        // Bei aktiver Rammbock-Option kommt Vorlage B für gelbe Ziele
        // ausschließlich aus dem Vorlauf - hier immer A verwenden, damit
        // nicht versehentlich ein zweiter B-Angriff (Maxloot) entsteht.
        if (
          optionRam &&
          farmIndex.hasOwnProperty('color') &&
          farmIndex.color == 'yellow'
        ) {
          template_name = 'a';
        }
        let template = data.farms.templates[template_name];
        ...
        // Auch hier: Belagerungswaffen separat prüfen, falls Vorlage B
        // (z.B. über die Maxloot-Option) Rammböcke oder Katapulte enthält.
        let siegeAvailable = hasSiege(data.villages[prop], template);
        ...
        if (unitsLeft && siegeAvailable && timeDiff && distance < optionDistance) {
          plan.counter++;
          ...
          subtractSiege(data.villages[prop], template);
